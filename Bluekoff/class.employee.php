...
class employee
{
		...
		<div class='container'>
			<div class='row'>
				...
				<table id='datatable' class='table table-bordered table-striped'>
					...
					<tbody>
					<?php
						$sql = "select `employee_info`.`id` as `id`, concat(`employee_info`.`fname`, ' ', `employee_info`.`lname`) as `name`, `payroll`.`salary` as `value` from `employee_info`, `payroll` where `payroll`.`emp_id` = `employee_info`.`id`";
						if ($searcher <> null) 
						{
	...
	function save_payroll()
	{
		...
		$sql = "insert into `acc_detail` set `acc_id` = '".$acc_id."', `typ_id` = '12', `typ` = '2', `detail` = 'Payroll Month#".$month."', `value` = '".$tt."'";
		$res = $conn->query($sql);

		$detail = "Reference From Payroll Month#".$month;
		$value = $tt;
		$sql = "insert into `payment` set `supplier_id` = '0', `date` = '".date('Y-m-d')."', `detail` = '".$detail."', `value` = '".$value."', `uid` = '".$_SESSION['uid']."', `ref_id` = '".$month."', `typ` ='2'";
		$conn->query($sql);
