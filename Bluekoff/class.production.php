...
class production
{
        ...
        <div class='container'>
            <div class='row'>
                ...
                <?php
					if (($acl == '2') or ($acl > '5')) {
				?>
                <input type='button' value='Add' onclick='window.open("index.php?option=production&task=add","_self")'>
                <?php
				}
				if ($acl > '3') {
				?>
                <input type='button' value='Print' onclick='window.open("productionint.php?cat=production&typ=all","_self")'>
                <?php
				}
				?>
                ...
                <table id='datatable' class='table table-bordered table-striped'>
                    ...
                    <tbody>
                        <?php
                        $a = 1;
                        $sql = "select `production`.`id` as `id`, `production`.`status` as `status`, `production`.`date` as `date`, `production`.`uaid` as `uaid`, `production`.`uid` as `uid`, `production`.`adate` as `adate` from `production` where '1' = '1'";
                        if ($searcher <> null) 
                        {
                        ...
                        if ($status > -1) 
                        {
                            $sql = $sql." and `production`.`status` = '".$status."' ";
                        }
                        $conn = new connect();
                        $res = $conn->query($sql);
                        while ($cdr = $res->fetch())
                        {
                            ...
                            echo "</td>";
                            echo "<td class='text-center'>";
							echo $conn->get_doc_code("PROD", $cdr['date'], $cdr['id']);
                            echo "</td>";
                            echo "<td class='text-center'>";
                            echo $cdr['date'];
                            echo "</td>";
                            ...
                            echo "<input type='button' value='".$ds."' onclick='confirm_del(\"index.php?option=production&task=del&id=".$cdr['id']."&stat=".$dss."&date=".$cdr['date']."\")' />";
							}
							if ($cdr['status'] == 1)
                            {
                            echo "<input type='button' value='Approve' onclick='window.open(\"index.php?option=production&task=del&id=".$cdr['id']."&stat=2&date=".$cdr['date']."\",\"_self\")' />";
							}
                            if ($cdr['status'] == 2)
                            {
            ...
            <div class='row'>
                ...
                <table id='datatable' class='table table-bordered table-striped'>
                    ...
                    <tbody>
                        ...
                        where `batch`.`status` = '2' and `batch`.`id` not in (select `production`.`batch_id` from `production` where `production`.`status` > 0)";
						//echo $sql;
                        $conn = new connect();
                        $res = $conn->query($sql);
                        while ($cdr = $res->fetch())
                        {
                            ...
                            echo "</td>";
                            echo "<td class='text-center'>";
                            echo "<input type='button' value='Select' onclick='window.open(\"index.php?option=production&task=edit&id=0&ref=".$cdr['id']."\",\"_self\")' />";
                            echo "</td>";
                            echo "</tr>";
                            $a++;
                        }
        ...
        <div class='container'>
            <div class='row'>
                ...
                <table class='table table-bordered table-striped'>
                    <tr>
                        <td colspan='2' class='text-center'><?php echo $head;?> production Data</td>
                    </tr>
                    <tr>
                        <td>User</td>
                        <td>
			...
			<?php
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    , `inventory`.`id` as stid
                    , (
                    select num from `production_detail` 
                    where `production_detail`.`status` > 0 
                    and `production_detail`.`inventory_id` = `inventory`.`id`
                    and `production_detail`.`prod_id` = '".$id."'
                    and `production_detail`.`typ` = '".$qq[$aq]."'
                    ) as num
                    from inventory 
                    where `inventory`.`status` > 0 having `num` <> ''";
					}
    ...
    function save()
    {
        ...
        if ($id > 0) 
        {
            ...
            $conn->query($sql);
			$code = $conn->get_doc_code("PROD", $date, $id);
			$conn->save_logs("Edit Production > ".$code, $_SESSION['uid'], $id);
        }
        else 
        {
            $sql = "insert into `production` set `date` = '".$date."', `uid` = '".$_SESSION['uid']."', `batch_id` = '".$ref."'";
            $id = $conn->query_lastid($sql);
			$code = $conn->get_doc_code("PROD", $date, $id);
			$conn->save_logs("Add Production > ".$code, $_SESSION['uid'], $id);
        }
        ...
        while ($a < $limit) 
        {
            if ($_REQUEST['num-'.$a] > 0) 
            {
                $sql = "insert into production_detail set prod_id = '".$id."', inventory_id = '".$_REQUEST['id-'.$a]."', num = '".$_REQUEST['num-'.$a]."', `typ` = '".$_REQUEST['typ-'.$a]."'";
                $conn->query($sql);
            }
            $a++;
        }
    ...
    function del()
    {
		...
		if ($stat == 2)
		{
			$sql = "update production set status = '".$stat."', `uaid` = '".$_SESSION['uid']."', `adate` = '".date('Y-m-d')."' where id = '".$id."' ";
            $conn->save_logs("Approve Production > ".$code, $_SESSION['uid'], $id);
		}
		else
		{
            ...
            if ($stat == 1)
            {
                $conn->save_logs("Active Production > ".$code, $_SESSION['uid'], $id);
            }
            elseif ($stat == 0)
            {
                $conn->save_logs("In-Active Production > ".$code, $_SESSION['uid'], $id);
            }
		}
        $conn->query($sql);
        header("location:index.php?option=production&task=def");
            ...
            <div class='row'>
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
								echo $conn->get_doc_code("PROD", $date, $id);
							?>
                        </td>
                    </tr>
                    <tr>
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    , `inventory`.`sale` as `production` 
                    from `inventory`, `production_detail`
				    where `inventory`.`status` > 0
                    and `production_detail`.`status` > 0
                    and `production_detail`.`inventory_id` = `inventory`.`id`
                    and `production_detail`.`prod_id` = '".$id."'";
                    $res = $conn->query($sql);
                    while ($cdr = $res->fetch()) 
