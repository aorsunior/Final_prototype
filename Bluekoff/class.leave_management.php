...
class leave_management
{
	...
	<div class='container'>
		<div class='row'>
			...
			<table id='datatable' class='table table-bordered table-striped'>
				...
				<?php
					$sql = "select *,
    							(select concat(`fname`, ' ', `lname`) from `employee_info` where `employee_info`.`id` = `leave_management`.`emp_id`) as `name`,
    							(select `users`.`name` from `users` where `users`.`status` = '1' and `users`.`id` = `leave_management`.`uaid`) as `uaid`,
    							`leave_management`.`adate` as `adate`
							from 
    							`leave_management`";
					if ($searcher <> null) 
					{
					...
					while ($cdr = $res->fetch())
					{
						...
						if ($cdr['status'] == 1)
						{
						echo "<input type='button' value='Approve' onclick='window.open(\"index.php?option=leave_management&task=del&id=".$cdr['id']."&stat=2\",\"_self\")' />";
						}
						echo "</td>";
						echo "</tr>";
					}
	...
	function del()
	{
		...
		if ($stat == 2)
		{
			$sql = "update leave_management SET status = '".$stat."', `uaid` = '".$_SESSION['uid']."', `adate` = '".date('Y-m-d')."' WHERE id = '".$id."' ";
			$conn->save_logs("Approve Leave#".$id, $_SESSION['uid']);
		}
		else
		{
			$sql = "update leave_management SET status = '".$stat."' WHERE id = '".$id."' ";
			if ($stat == 1)
			{
				$conn->save_logs("Active Leave#".$id, $_SESSION['uid']);
			}
			elseif ($stat == 0)
			{
				$conn->save_logs("In-Active Leave#".$id, $_SESSION['uid']);
			}
		}
		$conn->query($sql);
